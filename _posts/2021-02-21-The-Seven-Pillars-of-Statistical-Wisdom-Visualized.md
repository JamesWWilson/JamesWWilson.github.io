...
Where {% raw %} \$$ \bar{x}_1 $$ {% endraw %} and {% raw %} \( \bar{x}_2 \) {% endraw %}are the sample means, $s²$ is the pooled sample variance, $n_1$ and $n_2$ are the sample sizes and $t$ is a Student t quantile with $n_1 + n_2 - 2$ degrees of freedom.
